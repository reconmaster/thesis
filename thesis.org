...
\caption{Sagittal schematic of the axial FOV support for the
  dual-circle trajectory using the TrueBeam kV-imaging system
  geometry. In both figures, the upper half of the image (blue)
  represents the projective geometry from two opposing views at
  $\theta=0^{\circ}$ (dashed lines) and $\theta=180^{\circ}$ (solid lines) of the
  superior scanning trajectory. The lower half of the image (red)
  shows the inferior volume scanning configuration using the same
  convention of opposing views. The shaded region in the top an bottom
  portion represent the superior (red) and inferior (blue) volumes of
  support of a single circle. The top schematic demonstrates an axial
  separation $\left(d\right)$ of 10 cm, which for this geometry has
  complete support for both FDK and optimization-based reconstruction
  methods. The second schematic (b) shows an axial separation
  $\left(d\right)$ of 20 cm. As it can be seen by comparing these two
  images, the 20 cm spacing shown in (b) is the maximum axial coverage
  allowed by the two circles for which the image volumes are
  contiguous in the axial direction. For smaller axial offsets, there
  will be redundancy in the volume support which is indicated by the
  purple region in the top figure.}
...
axial coverage. This is partially due to the prevalence of
...
artifacts at the axial extremes of the reconstruction volume as shown
in Figure ([[ref:fig:opt_defrise]]). Another major reason for this limited
coverage is to the detector size which is restricted due to both cost
and engineering concerns. Given these limitations, we investigated if
it were possible to address the limited axial FOV problem using
current linac-mounted kV-imaging with the robust trajectory framework
described in [[id:ad13bdd9-4298-4534-979d-a019d80311a5][Generalized trajectory framework]].
...
there are some limitations to this approach. One drawback is that by
reconstructing the two circular volumes independently, neither scan
benefits from mutually-acquired information in the region of overlap
