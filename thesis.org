...
draw(det0--w,blue,Arrow3,L=Label("$w$", position=EndPoint, align=N));
...
transform3 det_shift=shift(5, -7, 3);
...
draw(Label("$\widetilde{X}_{\theta_g=0^{\circ}}$", 1),src--det_cent-(110,0,0), blue, arrow=Arrow3);

// transformed frame vectors
triple det0_uncal = point(detector_uncal, 1);
triple u_p = point(detector_uncal, 0) - det0_uncal;
triple v_p = point(detector_uncal, 2) - det0_uncal;
...
draw(det0_uncal--u_p,red,Arrow3,L=Label("$u'$", position=EndPoint, align=W));
draw(det0_uncal--v_p,red,Arrow3,L=Label("$v'$", position=EndPoint, align=N));
draw(det0_uncal--w_p,red,Arrow3,L=Label("$w'$", position=EndPoint, align=N));
...
#+CAPTION: Schematic represenation of a single projection view for the isocal phantom with the TrueBeam kV-CBCT scanning geometry shown using an orthographic projection. The blue detector and projected isocal fiducials correspond to the self-reported geometry from the imaging system. The red detector and projected fiducials illustrates how translation and rotation offsets create variations in the projected fiducials in the sinogram space. The top left corner of each detector repreents the origin of the detector coordinate system. The geometric translation and rotation offsets are exaggerated here for illustrative purposes.
...
$(\widetilde{X}_{\text{nom}})$ that transforms the simulated phantom
fiducials in room coordinates to projected spots in detector
coordinates. This is done on a view-by-view basis much like the single
view shown in Figure ([[ref:fig:geo_cal_schematic]]). Using the source
$(\vec{r}_s)$, detector $(\vec{r}_d)$, and gantry positions
$(\theta_g)$, the source and detector are rotated into the global
coordinate system of the isocal phantom using the gantry angle.
...
** Results
   ...
   :END:
*** Cost function
Here is an example of the cost function.
**** notes                                   :noexport:
- [[ipynb:(:url-or-port%20"https://remus.uchicago.edu:9999"%20:name%20"170603_virtiso_circ_smth_catphan/dynmag/calibs/calib_analysis.ipynb")][170603_virtiso_circ_smth_catphan/dynmag/calibs/calib_analysis.ipynb]]
** Conclusion
   :PROPERTIES:
