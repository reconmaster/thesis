...
#+CAPTION: Schematic from the Catphan manual off the CTP 404 sensitometry module.
#+ATTR_LaTeX: :width \textwidth
#+LABEL: fig:fov_ctp404
[[file:figures/fov/catphan_ctp404.png]]
...
module shown in Figure ([[ref:fig:fov_ctp404]]) aligned at imaging
...
CIRS torso phantom shown in Figure (). We aligned the phantom so that
the center of phantom was placed at the midpoint between the axial
position of the two circular components of the different trajectories.
We again acquired the three classes of scanning trajectories, but only
with a half-fan detector configuration as this larger phantom would
...
\centering
\begin{subfigure}[b]{0.45\textwidth}
  \includegraphics[height=1.75in]{figures/fov/cirs/cross_sectioned.jpg}
  \caption{}
  \label{fig:fov_cirs_sec}
\end{subfigure}
\qquad %add desired spacing between images, e. g. ~, \quad, \qquad,
% \hfill etc.
% (or a blank line to force the subfigure onto a new line)
\begin{subfigure}[b]{0.45\textwidth}
  \includegraphics[height=1.75in]{figures/fov/cirs/ed_setup.jpg}
  \caption{}
  \label{fig:fov_cirs_setup}
\end{subfigure}
