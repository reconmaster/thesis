...
** Introduction
   ...
   :END:
When performing tomographic image reconstruction it is critical that
the geometry of the scanning configuration is properly modeled.
This is true regardless of whether reconstruction is done with
analytic-based or optimization-based methods. If there is uncertainty
in where the projections come from, this will introduce
inconsistencies that will manifest as artifacts in the reconstruction.

With the additional complexity introduced with more exotic scanning
configurations, geometric calibration is of paramount importance in
obtaining a useful reconstruction. This is especially true when
working with trajectories where the object is moving in addition to
the source and detector during the scan.
...
parameters that would distort the projection of the phantom. In the
optimization-based approach, geometric parameters are varied to
improve the match between the projection of the modeled fiducials an
the detected fiducials in the sinogram.
...
Using the concepts previously developed optimization-based geometric
calibration
...
position of the beads on this phantom have a much higher tolerance
...
*** Calibration Method
    ...
    :END:
We designed a calibration work flow specifically for calibrating the
non-standard scanning trajectories we implemented on the TrueBeam
system with Developer Mode. As such, the nominal trajectory used to
initialize our calibration method was with the view-by-view geometry
information provided by the system for each projection. As this was
the uncalibrated initial guess with which we calculated our
reconstruction system matrix $\mathcal{H}$, the output of the
calibration subsequently improved the system matrix we used for image
reconstruction.

As with other calibration procedures, our task was to allow the
degrees of freedom in our scanning geometry to find the offsets that
improve the match between the scan of our calibration phantom with a
trajectory of interest, and the simulated projection of our phantom
model with that geometry. Assuming the accuracy of the modeled phantom
fiducials (an assumption that motivated our switch to the higher
tolerances of Varian's Isocal phantom), the phantom is first allowed
...
Using the nominal scanning geometry, the simulated phantom fiducials
are then forward projected onto the detector. These projected spots
are then matched to the detector spots in the sinogram. The $L_2$-norm
between the real and simulated projected spots is then calculated as
the cost function. The parameters corresponding to the geometric
degrees of freedom (DOF) are then allowed to iteratively vary by using
the Nelder-Mead simplex algorithm cite:lagarias_convergence_1998 to
search for the geometric offsets that minimize the $L_2$-norm cost
function.

*discuss coupling and how DOF order is choosen*

As with the other imaging calibration approaches, the crux of the task
is correctly determining the correct parameters for the projective
geometry. For each view, this entails correctly calculating the
projection matrix for that view. Within the 3D patient space, the
arrangement between the source, the detector, and the image origin
must first be correctly determined.

As the goal of our calibration method is to refine the view-by-biew
geometry of our forward projection model, we first start with an
initial guess of teh system geometry from the self-reported header
information from the TrueBeam system.

With these methods the projections of a to the modeled forward
projection of those fiducials with the nominal geometry

We implemented a calibration method

we developed for determining the geometric offsets related to these
non-circular scanning trajectories utilizes the parameter estimation cite:hartley_multiple_2003

To utilize the calibration method, the trajectory of interest is first
used to scan the phantom with the CBCT imaging system. Both the
projection data as well as the nominal acquisition geometry from the
projection headers are then used as input to the calibration.

this allows for easy spot
detection using a circular Hough transform cite:peng_detect_2007. By
comparing the distance of the centroids of the detected spots to those
of a model of the spiral pattern projected onto the detector from the
assumed trajectory geometry, a Nelder-Mead minimization can be used to
find the view parameters that minimize this cost.

For a new trajectory, this phantom can be scanned first to identify
any geometric offsets that are incorrectly reported in the TrueBeam
data headers. Though the self-reported position accuracy has been
found to be very good, we still need a way to verify the data header
information that we use to create the system matrix for new
scanning trajectories. This phantom provides a means of verifying this
for any of the scans implemented on the OBI.

The fiducials are subsequently forward projected using the same system
geometry with each projection view being determined from the nominal
geometry.

The projected spots in the data are found and compared to the
simulated forward projection of the modeled geometry.

Optimization is then used to minimize the L2-norm between the centroid
of the detected spot and that of the modeled projection by allowing
parameters of the system geometry to vary. Once the cost has been
