...
improve the match between the scan (observed projections) of our
calibration phantom with a trajectory of interest, and the simulated
projection of our phantom model with that geometry. Assuming the
accuracy of the modeled phantom fiducials (an assumption that
motivated our switch to the tighter tolerances of Varian's Isocal
...
search for the geometric offsets that minimize the $L_{2}\text{-norm}$
cost function. Once the cost has been minimized, the geometric offsets
are used as the calibration information for calculating the system
matrix $(\mathcal{H})$ for the image reconstruction.
